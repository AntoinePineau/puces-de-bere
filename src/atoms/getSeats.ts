...
export async function getSeats():Promise<Seat[]> {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await fetch('/api/tickets').then(response => response.json()).then(data => setExistingSeats(data));

      const existingSeatIds = new Map();
      if(existingSeats) existingSeats.forEach(seat => existingSeatIds.set(seat["label"], seat));
