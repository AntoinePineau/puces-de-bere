function addColumn(seats:any[], seatWidth:number, seatHeight:number, xCol:number, letter:string, startIndex:number, notAvailableSeatIds:any) {
  var seatId = `${letter}${startIndex}`;
  seats.push({ id: seatId, description: "1m20 linéaire avec angle sans table à l'intérieur", available: !notAvailableSeatIds.has(seatId), 
    price:1800, x: xCol, y: 1238, w: seatWidth, h: seatHeight*2 }); // A11
  for(var i=1;i<=6;i++) { // A12 ... A17
    seatId = `${letter}${startIndex+i}`;
    seats.push({ id: seatId, description: "1m20 linéaire sans angle sans table à l'intérieur", available: !notAvailableSeatIds.has(seatId), 
      price:600, x: xCol, y: 1286+(seatHeight+1)*i, w: seatWidth, h: seatHeight });
  }
  seatId = `${letter}${startIndex+7}`;
  seats.push({ id: seatId, description: "1m20 linéaire avec angle sans table à l'intérieur", available: !notAvailableSeatIds.has(seatId), 
    price:1800, x: xCol, y: 1636, w: seatWidth, h: seatHeight*2 }); // A18
}

export function getSeats(notAvailableSeats:Seat[]):Seat[] {
  ...
  const seats:Seat[] = [];
  const notAvailableSeatIds = new Set(notAvailableSeats.map(seat => seat.id));
  ...
  for(var i=1;i<=10;i++) { // A1 ... A10
    const seatId = `A${i}`;
    seats.push({ id: seatId, description: "1m20 linéaire sans angle sans table à l'intérieur", available: !notAvailableSeatIds.has(seatId), 
      price:600, x: col, y: 1186+(seatHeight+1)*i, w: seatWidth, h: seatHeight });
  }
  
  // Column A11 ... A28
  col = col + space;
  addColumn(seats, seatWidth, seatHeight, col, 'A', 11, notAvailableSeatIds);
  
  for (let charCode = 'B'.charCodeAt(0); charCode <= 'G'.charCodeAt(0); charCode++) {
    var letter = String.fromCharCode(charCode);
    // Column B1 ... B8 to G1 ... G8
    col = col + seatWidth+1;
    addColumn(seats, seatWidth, seatHeight, col, letter, 1, notAvailableSeatIds);

    // Column B9 ... B16 to G9 ... G16
    col = col + space;
    addColumn(seats, seatWidth, seatHeight, col, letter, 9, notAvailableSeatIds);
  }

  // Column H1 ... H8
  col = col + seatWidth+1;
  addColumn(seats, seatWidth, seatHeight, col, 'H', 1, notAvailableSeatIds);
