...
function addSeat(seats: Seat[], seatId: string, portrait: boolean, angle: boolean, xCol: number, yLine: number, interieur: boolean, existingSeatIds: any) {
  ...
  if (seatId == 'R17' || seatId == 'R18') {
    available = false;
    infoExposantFirstName = 'Maurice';
    infoExposantLastName = 'LUSSEAU';
  }
  ...
  const seat: Seat = {
    tierId: tierId, id: seatId, isAngle: angle, description: description, available: available,
...
export type Seat = {
  ...
  exposantFirstName: string; // Information about the exposant
  exposantLastName: string; // Information about the exposant
