...
export function getSeats():Seat[] {
  ...
  const seatWidth = 90; //2m40 (table 60cm + 1m80 d'espace derrière)
  const seatHeight = 45; // 1m20
  ...
  for(var i=1;i<=43;i++) { 
    seats.push({ id: `S${i}`, description: "1m20 linéaire sans angle sans table à l'intérieur", available: true, 
      price:600, x: 1310+(seatHeight+2)*i, y: 1810, w: seatHeight, h: seatWidth });
  }
  ...
  for(var i=1;i<=43;i++) { 
    seats.push({ id: `X${i}`, description: "1m20 linéaire sans angle sans table à l'intérieur", available: true, 
      price:600, x: 3640+(seatHeight+2)*i, y: 1860, w: seatHeight, h: seatWidth });
  }
  ...
  for(var i=1;i<=43;i++) { 
    seats.push({ id: `Y${i}`, description: "1m20 linéaire sans angle sans table à l'intérieur", available: true, 
      price:600, x: 3640+(seatHeight+2)*i, y: 1180, w: seatHeight, h: seatWidth });
  }
